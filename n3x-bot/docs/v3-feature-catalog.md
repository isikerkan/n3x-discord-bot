...
8. **[ ] Command-list channel — CONFIRMED NEXT (user, 2026-07-16)** (v3 system #9, `COMMAND_LIST_CHANNEL_ID` + `update_command_list_msg`) — a self-editing message in a dedicated channel listing all available commands (German), refreshed on ready. **Build DB/registry-driven** — enumerate the live command registry (`bot.commands` / groups incl. `!config`, `!gate`, `!admin`, gate/stat/achievement commands), NOT a hardcoded list — grouped by category with short German descriptions. Reuse the pieces we now have: self-editing message via the `channel_messages` store (`update_command_list` mirroring `update_gate_stats_embed`/`update_gate_input_help`); channel = new `command_list_channel_id`, **overridable** (add to `Settings` + `OVERRIDABLE_KEYS` + `RuntimeConfig` prop + `config_commands.CHANNEL_PURPOSES["command_list"]` + `.env.example`) so it's set via `!config channel command_list` — NO AMP field (config lives in .env + DB now). Refresh on_ready. (S/M)
