# N3X Bot — AMP Configuration Repository

This repository **is a CubeCoders AMP configuration repository**: the AMP
application template files live at the **repo root**, and the bot's own source
code lives in the [`n3x-bot/`](n3x-bot/) subfolder (see
[`n3x-bot/README.md`](n3x-bot/README.md) for bot/dev docs). AMP scans the root of
a configuration repository for its template, then fetches the bot code at runtime
into the instance's `./n3x-bot/` directory (the template's `App.RootDir`), so it
is browsable in AMP's file manager.

The template lets an admin configure the bot entirely in the AMP web GUI —
...
**Update** to clone/pull the bot — no baked-in clone command. The clone flattens
the whole repo into the instance base, so the code lands in `./n3x-bot/`; the bot
then runs as `python -u -m n3x_bot` with its working directory set to that folder.

## Installing via a Configuration Repository (recommended, native)

AMP can consume this repository natively: add its Git URL as a **Configuration
...
template files therefore live at **this repo's top level** (they were relocated
from `deploy/amp/`), while the bot's source code lives under `n3x-bot/`.

Config-repo layout (template files at repo root):
...
n3x-bot/                # the fetched bot's App.RootDir (source code, tests, docs)
```
...
# Add this configuration repository (use the raw git URL)
ampinstmgr AddRepository https://github.com/isikerkan/n3x-discord-bot.git
